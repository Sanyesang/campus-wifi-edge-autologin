...
当前只保留一个固定坐标点击工具：`CoordinateClicker.ps1`。它不通过程序接口检查 Wi-Fi、互联网、Edge 或认证结果，也不读取账号密码；这些动作由坐标序列完成。坐标序列从桌面右下角的网络入口开始，会依次打开网络面板、发起校园网连接、启动浏览器并操作认证页面。启动器窗口提供“移动 / 电信 / 联通”三个选项，默认预选电信。选择移动或电信时，第四次点击打开运营商选择框，第五次点击选中对应运营商：移动使用 `X=1336, Y=400`，电信使用 `X=1334, Y=424`；选择联通时跳过这两次点击，直接继续协议和登录操作。移动、电信流程共 8 次点击，联通流程共 6 次。工具等待 8 秒后开始点击，普通间隔 2 秒；第三次点击后会额外停留 8 秒，期间显示 80% 不透明度的“等待页面响应”大字倒计时，给浏览器打开认证页面留出更充足的时间。
...
- 旧版用户脚本配置中的默认运营商：电信 `@ctcc`。当前坐标点击器由启动窗口选择移动、电信或联通，默认预选电信。
...
3. 在启动器中选择校园卡运营商（移动、电信或联通），再点击“一键连接校园网”。
4. 工具会从右下角网络入口开始，自动联网并打开浏览器认证页面。移动和电信按 8 个坐标点击；联通跳过第四、第五次运营商选择点击，共 6 次点击。普通间隔 2 秒，第三次点击后等待 8 秒并显示倒计时。
5. 确认屏幕为 1920×1080（1080P），认证页面最终处于全屏、Edge 缩放 100% 的状态。
