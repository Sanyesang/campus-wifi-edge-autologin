...
当前只保留一个固定坐标点击工具：`CoordinateClicker.ps1`。它不通过程序接口检查 Wi-Fi、互联网、Edge 或认证结果，也不读取账号密码；这些动作由坐标序列完成。坐标序列从桌面右下角的网络入口开始，会依次打开网络面板、发起校园网连接、启动浏览器并操作认证页面。启动器窗口提供“移动 / 电信 / 联通”三个选项；首次使用没有默认运营商，必须先选定一个，选择会保存在当前 Windows 用户配置中。选择移动或电信时，第四次点击打开运营商选择框，第五次点击选中对应运营商：移动使用 `X=1336, Y=400`，电信使用 `X=1334, Y=424`；选择联通时跳过这两次点击，直接继续协议和登录操作。移动、电信流程共 8 次点击，联通流程共 6 次。工具等待 8 秒后开始点击，普通间隔 2 秒；第三次点击后会额外停留 8 秒，期间显示 80% 不透明度的“等待页面响应”大字倒计时，给浏览器打开认证页面留出更充足的时间。
...
- 旧版用户脚本配置中的默认运营商：电信 `@ctcc`。当前坐标点击器要求用户在启动窗口明确选择移动、电信或联通，并保存所选运营商供下次启动使用。

当前配置使用实测可响应的认证入口 `172.17.1.2/srun_portal_pc?ac_id=1&theme=pro`；旧门户域名和历史 IP 仍保留在绕过代理列表中，便于排错。

## 旧版 Edge 一次性设置（当前坐标工具不需要）

以下步骤只适用于已经失效的旧版自动认证方案；使用当前坐标点击器时，无需安装浏览器扩展或配置用户脚本。
...
3. 首次使用先选择校园卡运营商（移动、电信或联通）；选择会自动保存。没有选定运营商时，连接按钮和开机自启选项不可用。
4. 如需开机后自动连接，勾选“开机自启”。之后每次登录 Windows，工具会按已保存的运营商直接开始坐标点击流程，不需要再确认或按连接按钮。取消勾选即可移除本工具的开机启动项。
5. 手动连接时点击“一键连接校园网”。工具会从右下角网络入口开始，自动联网并打开浏览器认证页面。移动和电信按 8 个坐标点击；联通跳过第四、第五次运营商选择点击，共 6 次点击。普通间隔 2 秒，第三次点击后等待 8 秒并显示倒计时。
6. 坐标流程仅适用于 1920×1080（1080P）；认证页面必须全屏，Edge 缩放为 100%。

工具完成所选运营商对应的 6 次（联通）或 8 次（移动 / 电信）点击后，会自动关闭窗口并退出 PowerShell。它不会报告“认证成功”；请自行打开网页确认校园网是否已经可用。

开机自启在当前 Windows 用户的“启动”文件夹中创建快捷方式，不需要管理员权限或第三方依赖。它在用户登录 Windows 后运行；如果电脑停留在登录界面，流程会等到用户登录桌面后才开始。运营商选择保存在 `%LOCALAPPDATA%/JXNUCampusClicker/settings.json`，文件只记录运营商，不含账号或密码。
...
旧版本的“开机自启”只打开助手窗口，不会自动点击。当前坐标点击器已改为：先明确选择并保存运营商，再勾选“开机自启”，下次登录 Windows 后会直接按所选运营商运行点击流程。以上旧版“全自动联网”模式仍然失效，不要使用。
