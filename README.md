...
坐标点击器的运行前提是硬性的：**仅限 1920×1080（1080P）分辨率使用**；认证页面打开后必须处于全屏状态，Edge 浏览器缩放必须为 100%；系统显示缩放、任务栏位置、浏览器窗口布局和认证页面布局也必须与采集坐标时一致。任一条件变化，都可能导致点击偏移，需要重新采集坐标。
...
4. 确认屏幕为 1920×1080（1080P），认证页面最终处于全屏、Edge 缩放 100% 的状态；工具按 8 个坐标依次点击，普通间隔 2 秒，第三次点击后等待 8 秒并显示倒计时。

工具完成 8 次点击后会自动关闭窗口并退出 PowerShell。它不会报告“认证成功”；请自行打开网页确认校园网是否已经可用。
